...
class Screen(object):
    # @property 方式其实就是把一个get方法变成了一个属性，其属性名就是该get函数的名字，调用的时候也就是实例名.属性名
    # @property 同时也会产生一个setter的方法，可以用该函数名.setter来装饰set方法，同样的，就可以set方法变成了属性。
    # 总结来说@property 把get方法变成了一个只读属性，来供实例直接访问，而延伸出来@ xxx.setter把set方法变成了可写属性
    # 这样就能保证内部的变量没办法被外部修改，修改的只是set方法。
    # 使用@property 注意的是get 方法和set方法的名字必须保持一致。单使用property只有读属性。两个一起使用，该属性是可读写的、
    @property
    def width(self):
        return self.__width
